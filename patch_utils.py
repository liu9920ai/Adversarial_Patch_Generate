...
def test_patch(patch_type, target, patch, test_loader, model):
    ...
    for (image, label) in test_loader:
        ...
        #if int(predicted[0]) != int(label) and int(predicted[0].data.cpu().numpy()) != int(target):
        test_actual_total += 1
        ...
        _, predicted = torch.max(output.data, 1)
        if int(predicted[0].data.cpu().numpy()) == int(target):
            test_success += 1
    if  test_actual_total == 0:
        return 1
